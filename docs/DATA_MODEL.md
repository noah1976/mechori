...
車種カタログは、ユーザー車両の入力原文を失わずに参照IDを追加します。未確認仕様をマスタ値で自動補完しません。メーカー表記、翻字、同一市場名、市場別名称、車系統、世代を同じ文字列置換で処理しません。メーカーをまたぐ車両は単純な別名配列へ潰さず、`VehicleModelRelation`の種別に応じて検索範囲と確度を変えます。
