...
| `mechori.com` identity | PARTIAL / Human QA | Production domain and HTTPS are available; canonical-origin and legacy-host redirect rules are in the release candidate. Supabase / Google registration and login return still require owner verification. | Direct canonical URL, old-host path redirect, auth return, logout/login, and same Garage data from mobile/desktop. |
