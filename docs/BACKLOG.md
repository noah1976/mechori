# BACKLOG

## 運用ルール

- 1回の夜間作業では、原則としてREADYタスクを推奨順に1件だけ処理する。
- 成果物に指定された文書以外を変更しない。変更が必要になった場合はタスクを止め、依存関係または所有者確認事項として記録する。
- 完了条件をすべて満たした場合のみDONEへ移す。判断が必要な項目を推測で確定しない。
- 外部サービス接続、依存追加、データベース作成、デプロイ、課金、有料契約は、対応タスクと所有者承認なしに行わない。
- 状態は `READY`、`IN_PROGRESS`、`NEEDS_OWNER`、`BLOCKED`、`DONE` のいずれかとする。

## 暫定決定

- 初期公開は日本を中心とした少人数の招待制ベータとする。
- 内部設計とUIは日本語・英語対応を前提とする。
- 投稿の公開デフォルトは非公開とする。
- ユーザーが内容を確認し、明示的に公開する。
- 初期ベータでは、公開ナレッジ化する投稿を運営者が確認できる方式を採用する。
- 外部サービスはローカルのモック版と設計確認後、必要になったものだけ導入する。
- MVPでは有料プラン、決済、広告を導入しない。
- 将来の権限差と広告枠のみ拡張可能にする。

暫定決定を公開仕様または外部契約へ進める際は、関連文書との整合と所有者確認を行います。

## READY

### MECH-001 初期MVPのユーザーストーリー

- 優先度: P0 / 推奨順序1
- 状態: READY
- 目的: 招待制ベータで誰が何を達成するかを、実装判断に使える粒度で定義する。
- 成果物: `docs/USER_STORIES.md`
- 完了条件: オーナー、メカニック、運営者の主要ストーリー、受入観点、対象外が日本語で整理され、日本語・英語UI、非公開デフォルト、ユーザー確認、運営確認の前提が反映されている。
- 依存タスク: なし
- 所有者確認の要否: 不要。未確定事項は確定せず `NEEDS_OWNER` として記録する。

### MECH-002 主要画面一覧

- 優先度: P0 / 推奨順序2
- 状態: READY
- 目的: MVPのユーザーフローと画面責務を明確にする。
- 成果物: `docs/SCREENS.md`
- 完了条件: Webの主要画面、利用者、目的、主要操作、状態、権限制御、日英対応、安全・確認導線が一覧化され、将来のExpo画面との共通・固有部分が区別されている。
- 依存タスク: MECH-001
- 所有者確認の要否: 不要。ブランド表現や初期ログイン方式は仮決めしない。

### MECH-003 初期データモデル

- 優先度: P0 / 推奨順序3
- 状態: READY
- 目的: 実装前に、プライバシー、安全、権限、出典、確認、監査に必要な概念と関係を定義する。
- 成果物: `docs/DATA_MODEL.md`
- 完了条件: 概念モデル、識別子、主要属性、関係、状態遷移、個人記録と共有ナレッジの分離、出典、確度、AI処理、確認、訂正履歴、出力・削除・匿名化の考え方が記載され、物理DBや特定Providerに固定されていない。
- 依存タスク: MECH-001、MECH-002
- 所有者確認の要否: 必要。FIAT Barchettaの仕様分類粒度と初期管理者運用に関わる箇所は承認対象とする。

### MECH-004 危険領域タグ

- 優先度: P0 / 推奨順序4
- 状態: READY
- 目的: 誤情報が重大事故につながり得る整備領域を識別し、表示・確認・公開判断に利用できるようにする。
- 成果物: `docs/HAZARD_TAGS.md`
- 完了条件: タグ候補、重大度、適用基準、警告表示、公開・運営確認への影響、専門家確認が必要な範囲、誤判定時の訂正方法が整理されている。
- 依存タスク: MECH-001、MECH-003
- 所有者確認の要否: 必要。公開前に整備・法務の専門家確認対象を所有者が決める。

### MECH-005 日本語／英語の基本用語集

- 優先度: P1 / 推奨順序5
- 状態: READY
- 目的: UI、データ設計、検索、安全表示で使う基本用語の日英対応を統一する。
- 成果物: `docs/GLOSSARY_JA_EN.md`
- 完了条件: 車両、整備、部品、症状、状態、公開、安全、AI・OCRに関する基本語について、日本語、英語、定義、使用上の注意が記載され、未確定の車種固有分類が明示されている。
- 依存タスク: MECH-003、MECH-004
- 所有者確認の要否: 不要。専門用語の最終確定が必要な箇所はレビュー待ちとして明示する。

### MECH-006 外部サービス導入前チェックリスト

- 優先度: P1 / 推奨順序6
- 状態: READY
- 目的: 外部サービスを必要性、データ、費用、法務、安全、撤退可能性の観点で評価できるようにする。
- 成果物: `docs/EXTERNAL_SERVICE_CHECKLIST.md`
- 完了条件: Supabase、Vercel、OCR、AI、翻訳、Storage、メール、Expo関連サービスを評価でき、ローカルモック、送信データ、保存期間、利用量、費用上限、障害・停止、データ移行、環境分離、所有者承認の確認項目が含まれている。
- 依存タスク: MECH-003
- 所有者確認の要否: 必要。実サービスの契約・接続・有料枠利用は別途明示承認を得る。

## NEEDS_OWNER

### MECH-101 初期ログイン方式

- 優先度: P0
- 状態: NEEDS_OWNER
- 目的: 招待制ベータに適した本人確認、利便性、運用負荷のバランスを決める。
- 成果物: 認証方式の決定記録
- 完了条件: 候補比較、招待方法、アカウント復旧、日英UI、個人情報、運用者対応を確認し、所有者が方式を承認している。
- 依存タスク: MECH-001
- 所有者確認の要否: 必要

### MECH-102 FIAT Barchettaの仕様分類粒度

- 優先度: P0
- 状態: NEEDS_OWNER
- 目的: 車両差異を安全に扱える最小分類を決める。
- 成果物: 分類方針の決定記録
- 完了条件: 年式、地域、エンジン、型式、改修差等の候補と情報源を比較し、初期粒度を所有者が承認している。
- 依存タスク: MECH-003
- 所有者確認の要否: 必要

### MECH-103 ロゴ、カラー、タイポグラフィ

- 優先度: P1
- 状態: NEEDS_OWNER
- 目的: MECHORYのブランドと安全表示に適した視覚方針を決める。
- 成果物: デザイン方向性の決定記録
- 完了条件: ロゴ、配色、文字体系、日英可読性、アクセシビリティ、安全警告との区別を確認し、所有者が承認している。
- 依存タスク: MECH-002
- 所有者確認の要否: 必要

### MECH-104 モノレポ構成

- 優先度: P0
- 状態: NEEDS_OWNER
- 目的: Web、将来のExpoアプリ、共通ロジックの配置と所有境界を決める。
- 成果物: リポジトリ構成の決定記録
- 完了条件: 単一構成との比較、共有範囲、ビルド・テスト・デプロイ境界を確認し、所有者が構成を承認している。
- 依存タスク: MECH-001、MECH-002、MECH-003
- 所有者確認の要否: 必要

### MECH-105 パッケージマネージャ

- 優先度: P0
- 状態: NEEDS_OWNER
- 目的: 再現性、ワークスペース対応、運用負荷を踏まえて管理方式を決める。
- 成果物: パッケージマネージャの決定記録
- 完了条件: 候補、固定方法、CI・ホスティング互換性を比較し、所有者が選択を承認している。
- 依存タスク: MECH-104
- 所有者確認の要否: 必要

### MECH-106 初期管理者運用の詳細

- 優先度: P0
- 状態: NEEDS_OWNER
- 目的: 公開ナレッジの確認、通報、訂正、緊急非公開化を少人数で運用できるようにする。
- 成果物: 初期管理者運用の決定記録
- 完了条件: 担当者、権限、確認基準、対応時間、エスカレーション、記録、代理対応を定め、所有者が承認している。
- 依存タスク: MECH-001、MECH-002、MECH-004
- 所有者確認の要否: 必要

## BLOCKED

以下は実装タスクではなく、所有者承認と前提資料が揃うまで開始しない外部ゲートです。

| タスクID | 優先度 | 状態 | 目的 | 成果物 | 完了条件 | 依存タスク | 所有者確認の要否 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| MECH-201 | P1 | BLOCKED | 外部サービス接続 | 接続・運用記録 | 設計確認、アカウント、料金、データ取扱い、停止手順が承認済み | MECH-006 | 必要 |
| MECH-202 | P1 | BLOCKED | 本番デプロイ | 公開判定記録 | ドメイン、環境分離、法務・安全・復旧確認が承認済み | MECH-006、MECH-106 | 必要 |
| MECH-203 | P2 | BLOCKED | 決済導入 | 決済設計 | MVP終了後に料金、税務、返金、事業者が承認済み | 将来の収益検証タスク | 必要 |
| MECH-204 | P1 | BLOCKED | AI利用開始 | AI利用判定記録 | 送信データ、費用、表示、確認、評価、停止手順が承認済み | MECH-006 | 必要 |
| MECH-205 | P0 | BLOCKED | 法的文書確定 | 公開用法的文書 | 専門家確認または所有者判断が完了 | 公開仕様確定後 | 必要 |

## DONE

### MECH-000 初期プロジェクト設計

- 優先度: P0
- 状態: DONE
- 目的: コード実装前のプロジェクト方針とCodex恒常ルールを定める。
- 成果物: 初期設計文書一式、`AGENTS.md`
- 完了条件: 初期文書と恒常ルールが作成され、コード、依存、データベース、外部接続を追加していない。
- 依存タスク: なし
- 所有者確認の要否: 完了済み
