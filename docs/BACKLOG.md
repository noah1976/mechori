...
- 優先度: P1
- 状態: READY
- 目的: 実検出Provider導入前に、候補の追加・修正・誤検出確認と公開不可理由の操作性をローカルで検証する。
- 成果物: DEMO画像だけを使う確認画面、状態テスト
- 完了条件: 未確認、検出失敗、原本、未マスク候補が公開不可と表示され、外部通信や実画像保存を行わない。
- 依存タスク: MECH-014
- 所有者確認の要否: 必要。実画像検出はMECH-206まで行わない。
...
- 成果物: ローカルDataProvider、追加・編集・表示、移行用DEMOアダプター、テスト
- 完了条件: 1イベントに複数作業を登録でき、メーター交換回数に上限がなく、表示値の逆行を拒否せず経緯確認へ送る。
- 依存タスク: MECH-003、MECH-012
- 所有者確認の要否: 必要。現行localStorageデータの破棄・移行方針を表示する。
