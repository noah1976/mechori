...
- 実装: Authenticated `/` Passport入口、primary navigationからのFeed退避、`/home`での既存Feed維持、Vehicle 0/1/複数、全任意入力、private AppData保存、Owner preview/edit、既存Feedback再利用、明示的なWorkshop share作成・copy/share・revoke、unauthenticated `/p/[token]`限定projection、`noindex / nofollow`。新規／明示更新shareは対象VehicleのGarage整備履歴をID・cost・private notesなしのsnapshotで持ち運び、既存shareは自動拡張しない。Workshop返却は1 Visit内へ1〜20件のService Itemを追加でき、Ownerは原文を保持したままItem追加・修正・削除、dismiss、既存Garage Maintenanceへの明示追加までを一周できる。
...
- Persistence: `202609170001_alpha_passport_shares.sql`、`202609200001_alpha_passport_service_reports.sql`、`202609210001_passport_service_items.sql`、`202609210002_passport_history_projection.sql`は`mechori-alpha`へ適用済み。history migrationは既存rowをv1のまま保ち、履歴同意後だけv2 snapshotへ更新するadditive migrationである。
...
- 状態: NEEDS_OWNER
- 目的: 記録供給を増やす前提を再検証し、資料整理・相談準備・結果回収が本人便益と他者Evidence再利用を生むかを判定する。
- レビュー成果物: [KNOWLEDGE_ACQUISITION_REVIEW_2026-09.md](KNOWLEDGE_ACQUISITION_REVIEW_2026-09.md)。`REVIEW_READY / ADOPTION_PENDING / EXPERIMENT_NOT_STARTED`。8方式×20観点、手動試験、失敗・反転条件まで文書化済み。
- 次の候補: Founder実資料5枚以内で調整し、外部Owner最初の2人で入口gateを確認、最大4人・35日以内・Founder計12時間で一件の相談準備と後日結果・再利用を追う。詳細はレビュー§18–20。
- 完了条件: 採用／不採用を理由付きで記録。採用する場合は7問を行動事実と欠測・支援量で判定し、A本人履歴／B相談支援／C共有Evidenceの継続・縮小・停止を別々に決める。レビュー作成だけでは実験完了にしない。
- 依存・境界: 利用機会、実資料、権利、参加同意、既存の安全な私的受渡し。MECH-045/046/047の本番実装を試験の前提にしない。Quick Recordのα共有固定へ原本を投入しない。新規AI送信・課金・サービス接続・DB・コードは対象外。
- 所有者確認の要否: 実験の採用と実施は次タスクで判断。本レビューは手動実験を開始せず、既存戦略を削除・置換しない。
