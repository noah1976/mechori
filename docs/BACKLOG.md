...
- 現在地: `@supabase/ssr`、ランタイム設定、ブラウザ・サーバーClient、Next.js Proxy、α用初期マイグレーション、外部設定手順を追加済み。OAuth callback、招待API、Workspace Adapter、RLS統合テストは未実装。
...
- 完了条件: M0〜M2の指標、算出式、イベント候補、個人情報を増やさない計測方針、手動集計方法を定義済み。外部分析SDKは導入しない。
- 依存タスク: MECH-026
