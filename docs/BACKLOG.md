...
- 依存タスク: MECH-015、今回実装したGarage記録の発生時期モデル
- 所有者確認の要否: データモデルとローカル実装は不要。本番DB変更は必要。
...
- 成果物: 3〜5工場のインタビュー、工場内限定・対応経験・許諾ナレッジの手作業検証、工場間相談の価格仮説、最初の帳票ワークフロー候補
- 完了条件: 共有可能範囲、相談対価、知識クレジット、既存業務ソフトとの二重入力、最初に試す単一帳票を実案件から決める。対象国の制度適合をAIだけで確定しない。
- 依存タスク: MECH-029、`PROFESSIONAL_PLATFORM.md`
