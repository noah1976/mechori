...
現行リポジトリだけで、実装・テスト・本番反映・人間QAの全てを確認できる項目はありません。人間QAの記録がリポジトリに残っていない機能を、推測で `[x]` や完了扱いにしていません。
...
- **P-051 いいね追加・解除**: 一覧・詳細の状態更新、保存失敗時の復元、二重送信抑止を実装・テスト済み。人間QAでは一覧と詳細で、追加、再読込後の維持、解除を確認する。
- **P-061 投稿者導線**: 投稿者アイコン・表示名から公開プロフィール、車名・車両画像から車両プロフィール、本文から投稿詳細へ分離。人間QAではモバイルで誤遷移とキーボードfocusを確認する。
- **P-064 投稿のお題**: 固定5件と書き方ヒント、自由投稿入口を実装・テスト済み。人間QAではお題選択、ヒント表示、未選択での投稿を確認する。
- **P-066 詳しい記録の編集保存**: 本文、日付、写真、写真説明、公開範囲、AI同意、関連整備記録の保存エラー処理を実装・テスト済み。人間QAでは写真あり・なし、関連記録あり・なしで既存データを失わないことを確認する。
- **P-069 既存共有写真の表示**: Storage共有・署名URL・再試行の診断と修正経路、関連テストは存在するが、過去に本番403が発生したため未解決扱いを維持する。人間QAでは保存済み「近所ドライブ」の2枚を別端末、再読込、再読み込み操作で確認し、表示できるまで完了にしない。
...
- **初回表示高速化**: 認証後Workspaceの重複・直列取得を整理し、初期Hydrationテスト済み。人間QAでは愛車なし、愛車あり・投稿あり、通信失敗時の停止状態を確認する。
...
- **P-073 アプリ内通知一覧・未読バッジ**: 未実装。通知を必要とする機能が増える前に、アプリ内通知の最小仕様を検証する必要がある。
...
1. P-069の保存済み共有写真を別端末・再読込・再試行で確認し、Storage 403が再発しないことを人間QAで確定する。
2. P-066、P-071、P-072を写真あり・写真なし・通信失敗を含めて実機確認し、投稿の保存から再利用までを通す。
3. 2人以上のα参加者で、Google招待、プロフィール検索、ユーザーフォロー、車両フォロー、相互フォロー、重複なしフィードを確認する。
4. 管理者／非管理者のRLS、監査、Owner Plus、フィードバック、計測境界を確認する。
5. 初回表示と車両登録から最初の記録までの迷いをαテスターから集め、次の実装優先度を決める。
...
- 本文保存成功と写真表示成功は別経路であり、P-069は本番実機での写真取得確認が終わるまで解決済みとしない。
