...
- **P-074 UI/UX総点検・再設計**: `docs/ux/`のUX設計文書7本は作成済み。プロダクトオーナーのレビューと未決事項の確定途中で、UI実装は未着手のためPARTIALとして扱う。
...
1. P-077の本番実機QA（各投稿一覧からの初回タップ、直接URL、再遷移）。
2. P-074のUX設計レビューと下部ナビ・記録導線の試験実装。
