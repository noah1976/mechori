...
- 更新日時: 2026-09-18
- 対象ブランチ: `codex/23-passport-prototype`
...
### Passport prototype checkpoint

- **状態: `PASSPORT_PROTOTYPE_READY` / `EXPERIMENT_NOT_STARTED`**。Authenticated `/`を愛車パスポート入口とし、既存Following Feed Homeは`/home`へ移して共通navigationから引き続き利用できる。未ログイン`/`とOAuth callbackの既定return pathは維持する。
...
- Workshop共有は明示的opt-inで、Passport専用の高entropy token、hash保存、限定projection、revokeを持つadditive migrationを用意した。`/p/[token]`は未ログイン閲覧可能かつ`noindex / nofollow`で、email、auth ID、内部ID、他の記録、private notesは返さない。
- ローカル390px QAでは、初期入力、任意項目保存、完成表示、Owner preview、共有作成、未ログイン共有閲覧、共有停止後の旧URL拒否、`/home`と`/garage`への導線、横overflowなしを確認した。これはProduction実機QA完了を意味しない。
- このbranchではmigrationを本番へ適用せず、Production mergeも行わない。Deploy Previewで共有まで試すにはmigration適用が先に必要であり、iPhone Safari、Android Chrome、実アカウント、incognito、revoke後の旧URL拒否はHuman QA pendingである。
- この実装は「愛車パスポート戦略が成功した」ことを意味しない。作成完了率、工場へ見せたい意向、欠けている情報、不要な入力をα実験で確認する。
