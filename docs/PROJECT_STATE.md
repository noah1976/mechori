...
2026-07-15時点で、MECHORYは初期設計からローカルプロトタイプ検証へ進みました。
...
- 構造化データはlocalStorage試作スキーマv5、メディア本体はIndexedDBへ分離し、旧Journalを順序付き本文ブロックへ自動移行する
- ユーザー実ファイルを外部送信せず、自動マスキング・メタデータ除去・動画確認が未実装の間は非公開投稿だけ許可する
- localStorage試作スキーマv5へ移行し、Journal本文を段落・見出し・引用・画像・動画の順序付きブロックとして保持する
...
1. 所有者に、note型Garage Journal編集、新しいホーム、根拠付き検索要約の方向性を操作確認してもらう。
2. 所有者に、履歴サマリー、工場向け車両履歴、抽出結果確認DEMOの方向性を確認してもらう。
3. 実物資料が用意できた時点で、3〜5ページとナンバーが写る画像・短い動画を使って取込・マスキング設計を検証する。
