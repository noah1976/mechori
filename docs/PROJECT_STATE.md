...
- Journalの初期値を非公開のまま維持し、本人が選んだ投稿本文と表示用車名だけを参加中のP0・αテスターへ共有するフィードを実装。写真・動画、関連整備記録、内部ID、ナレッジ抽出同意は共有データへ含めない
...
- α参加者向けJournal共有用`202607290002_alpha_shared_journals.sql`はローカル実装のみで、本番Supabaseには未適用。適用前に、別利用者へ文字情報だけが届くこと、写真・関連整備記録・内部IDが届かないこと、未ログイン・参加停止時の拒否を確認する。
...
- 新規記録は非公開を初期値とする。P0のJournal共有は本人が選んだ文字情報だけを参加中のαテスターへ失効可能な写しとして共有し、写真・関連整備記録・ナレッジ化は別の確認ゲートを維持する。
