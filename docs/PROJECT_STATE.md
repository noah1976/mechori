...
- 愛車パスポートの主要価値を「今回の伝言」だけでなく、このVehicleの過去整備履歴を次のWorkshopへ持ち運べることとして試験する。新規shareとOwnerが明示更新したshareだけが、対象Vehicleの`MaintenanceRecord.actions[]`をID・cost・private notes等を除いた専用snapshotとして含む。既存shareはdeployだけで履歴付きへ拡張しない。
...
- α projectは`mechori-alpha`（project ref `bodccmvwiqqghxzhezwt`）の1件だけであることを確認した。既存3 migrationに続き、整備履歴snapshot用の`202609210002_passport_history_projection.sql`を2026-09-21に適用済み。既存share 1件はversion 1／historyなしのまま維持され、version 2への自動拡張は0件である。anonのshare table直接SELECT／INSERTは不可で、限定public read RPCだけを実行できる。iPhone Safari、Android Chrome、実アカウント、incognito、既存share明示更新、revoke前後、preview authはHuman QA pendingである。
...
- **PR #26 boundary**: [PR #26](https://github.com/noah1976/mechori/pull/26)のdescriptionとdiffをread-onlyで確認した。監査時点でOPEN・未merge。過去MaintenanceRecordの限定projectionと返却後の共有更新は、LINEとの差を作る現在検証中の最新Product Hypothesisとして扱い、`main`の実績や市場検証済み価値とは扱わない。PR #26の変更・mergeは行っていない。
