...
- **P-078 プロフィール画像の主要画面表示**: private Storageの認証済みダウンロードから生成したBlob URLで表示する方式は実機確認済み。今回、表示方式を変更せず、共通AvatarのHooks lint違反を修正した。表示回帰の人間QA前はSHIPPED_NEEDS_QAとして扱う。
...
- **P-074 PCナビ修正**: PCでは左サイドバーへ全メニューを統合し、ハンバーガー、drawer、overlay、FABを使わない方針を採用済み。現行コードへの反映と人間QAは未着手。
- **P-075A つながり**: 自分と他人のフォロー中／フォロワー、双方向フォローから導出する関係状態、公開Garageへの遷移、一覧内フォロー／解除、自分がフォローした車両の一覧を実装した。取得は既存のユーザーフォローとWorkspace内の車両フォローを再利用し、active member・公開中車両・ブロック境界だけを返す追加の読み取りRPCをマイグレーションとして用意している。マイグレーションの本番適用と配信前のためPARTIALとし、適用・本番反映後も人間QA前はSHIPPED_NEEDS_QAとする。
