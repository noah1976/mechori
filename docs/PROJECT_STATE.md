...
- 初期管理者は既存の`owner`ロールで運営画面へ入り、管理画面から`admin`、`moderator`、`support`を理由付きで付与できる。管理画面に直近の監査履歴を閲覧するUIとDB関数を追加済みだが、別権限利用者での表示境界確認が必要。
...
- 監査履歴閲覧用`202608030002_admin_audit_history.sql`とプロフィール画像用`202608030003_alpha_profile_images.sql`は本番Supabaseへ適用済み。owner/admin以外の監査履歴拒否、本人による画像変更・削除、別利用者表示、ブロック・参加停止・未ログイン時の取得拒否は人間による統合確認が必要。
