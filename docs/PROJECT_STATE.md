...
- **mobile P1修正とQA**: iPhone Safariの再現を受け、日付input自身ではなく保存後enrichment form内のfieldsetが`max-inline-size`だけでauto幅になっていた点を修正した。fieldsetをgrid track幅の`inline-size: 100%`かつ`border-box`にし、form childの`min-inline-size: 0`を明示した。コード・自動検証後も、320 / 375 / 390 / 430pxでの日付、Bottom Sheetとkeyboard / bottom navigation、本文のみ・共有写真付き保存、enrichmentのSkip / Close / failure、下書きを人間QAするまでSHIPPED_NEEDS_QAとする。
