...
- **P-076 記録本文内の写真表示改善**: 記録のcontentBlocks順を維持し、本文写真を自然なアスペクト比で表示する実装・関連テスト・本番反映済み。人間QAでは写真と本文の順序、縦横比、写真説明、スマートフォンとPCの表示を確認する。
...
- **P-078 プロフィール画像の主要画面表示**: 非公開Storageの署名URLを共通Avatarから表示し、メニュー、Garage、公開プロフィール、投稿一覧・詳細へ接続。実装・関連テスト・本番反映済みだが、人間QA前のためSHIPPED_NEEDS_QAとして扱う。
...
5. P-075つながり管理、P-076記録写真表示改善、P-070初回表示速度の追加計測。
