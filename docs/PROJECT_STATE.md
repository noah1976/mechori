...
2. Garage Journal、車両プロフィール、フォローフィードをローカルDEMOへ追加し、整備記録と楽しさの両立を検証する。
3. 現行モックでピンとこなかったホームを、症状検索、フォローフィード、記録追加の優先順位が分かる構成へ再設計する。
4. 実物資料が用意できた時点で、3〜5ページとナンバーが写る画像を使って取込・マスキング設計を検証する。
5. 招待制ベータへ進む前に、初期ログイン方式と管理者運用を決定する。
