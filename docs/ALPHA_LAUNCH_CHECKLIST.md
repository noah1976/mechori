...
- `[~]` 投稿を編集し、本文、日付、写真、関連整備記録を保持したまま保存する
- `[~]` いいねを追加・解除し、再読込後も状態が維持されることを確認する
...
- `[~]` P-069: 既存「近所ドライブ」の写真2枚を別端末で表示する
- `[~]` P-069: ページ再読込後も写真を表示する
- `[~]` P-069: 「再読み込み」で署名URL取得を再実行し表示する
...
- `[~]` 初回表示を愛車なし・投稿あり・通信失敗で確認する

## 7. 未実装・延期

- `[ ]` P-073 アプリ内通知一覧・未読バッジ
- `[-]` ネイティブアプリのプッシュ通知
- `[-]` アプリアイコンの未読件数
- `[-]` AIナレッジの本格提供
