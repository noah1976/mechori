...
- `[ ]` P-075A: 自分・他人のフォロー中／フォロワー、相互フォロー表示、公開Garageへの遷移、一覧内フォロー／解除、フォロー中のクルマを確認する（追加RPCマイグレーションの本番適用・配信・人間QA待ち）
...
- `[~]` フィードバックを「良かった／迷った／動かなかった／欲しい／その他」の独立ボタンで選び、運営側で未評価・status・種別・期間を整理する（GPT用Markdown一括出力は将来拡張）
- `[~]` 管理画面へ管理者だけが入れる
...
- `[ ]` P-074 PCナビ修正（左サイドバーへ全メニューを統合し、PCのハンバーガー、drawer、overlay、右下FABを非表示にする。実装・QA未着手）
