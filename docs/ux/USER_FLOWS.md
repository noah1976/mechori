...
flowchart LR
  ...
  J --> K[Ownerにpending表示]
  K --> L{Owner確認}
  L -->|修正して承認| M[既存Maintenance履歴へ追加]
  L -->|今回は追加しない| N[dismiss]
  M --> O[Garageで確認]
  F --> H[共有を停止]
```
...
| 項目 | 内容 |
| --- | --- |
| 状態 | Roundtrip prototype ready / Experiment not started / Human QA pending |
| 開始条件 | authenticated `/`。Vehicle 0台では既存愛車登録へ進み、完了後に`/`へ戻る |
| 主要ステップ | Vehicleを確認 → 任意項目を分かる範囲だけ入力 → 保存 → feedbackは送る／skip → Owner preview → 明示的に共有link作成 → 工場が未ログイン閲覧・任意項目を返却 → Ownerが原文を確認・必要なら修正 → 履歴追加またはdismiss → Garageで確認 |
...
| CURRENT | `/journal/new`と車両別作成はUniversal Composerを既定とし、Vehicle、本文、任意写真、保存から始める。日付、公開範囲、種別、整備者情報、下書き、完了導線がある |
| PROPOSEDの主要ステップ | 「記録を残す」 → 車両確認 → 「このクルマに、何を残す？」を1 tapで選ぶ → すぐ自由に書く → 写真は任意 → 保存 → 完了 → 必要なら「記録を詳しくする」。将来は同じ経緯へ「続きを残す」が、初回保存と後日追記の両方を扱う |
| 迷いやすい分岐 | Vehicleが複数ある場合、未確認Providerと工場公式情報の違い、公開範囲。クイック／詳しいの分類は投稿開始前に強制しない |
| 失敗状態 | バリデーション、通信、写真共有の失敗。保存されていないことを明示し、入力と下書きを保持する |
| 完了状態 | 愛車名と記録がVehicle historyへ加わったことを返す。Garage、記録詳細、次の記録、ホームへ進める。詳細入力や将来の継続操作を初期保存の成功条件にしない |
| 改善が必要な箇所 | P-086のiPhone QAで、30秒程度で本文のみまたは本文＋写真を保存できるか、キーボード時の保存操作、詳細設定、写真失敗時の入力維持を確認する。写真は記録の公開範囲へ従うと一度だけ説明する |
...
| CURRENT | 下書き復元、フォロー中の記録、P-075Aつながりを実装済み。P-073はLike、新規Follower、Follow対象の新規公開記録を一覧化し、未読badge、個別／一括既読、権限外のsafe stateを提供する。Push／Emailは未実装 |
...
| 改善が必要な箇所 | Web通知の本番DB適用・配信後に、ホーム新着との役割差、通知量、再訪動機を人間QAする |
...
| --- | --- |
| 開始条件 | 「探す」から検索条件を入力する |
| CURRENT | 検索条件フォーム末尾の「この条件で探す」をsubmitとして使い、入力欄からEnter／キーボード検索でも同じ処理を実行する。検索中は記録作成FABを表示しない。キーワード送信時は公開中の人・クルマも検索し、プロフィール／Vehicleページへ進むか、それぞれを独立してFollowできる |
| PROPOSEDの主要ステップ | 条件入力 → この条件で探す → 人・クルマ・事例の結果を確認 → Vehicle／Garage／記録詳細へ進む、または必要時の記録作成へ進む |
