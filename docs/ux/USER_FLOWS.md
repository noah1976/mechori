...
| CURRENT | Googleログイン、招待、初回3ステップ案内、プロフィール設定、車両登録、Garage、記録作成の経路がある |
| PROPOSEDの主要ステップ | 未ログインHomeで目的を理解 → 招待・ログイン → 3ステップ案内（あとで可能） → 表示名の確認 → 愛車登録 → 任意のお題 → 最初の記録 → 完了 → マイガレージ |
...
| 改善が必要な箇所 | P-081で短い価値説明、3ステップ案内、Homeチェックリストを追加した。人間QAでは約5秒で目的と最初の行動を理解できるか、既存ユーザーと招待return-toを確認する |
