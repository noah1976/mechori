...
| CURRENT | Googleログイン、招待、プロフィール設定、車両登録、Garage、記録作成の経路がある |
| PROPOSEDの主要ステップ | 招待・ログイン → 表示名の確認 → 愛車登録 → 任意のお題 → 最初の記録 → 完了 → マイガレージ |
...
| 改善が必要な箇所 | 認証、プロフィール、車両登録、最初の記録を別サービスに見せない。車種不明でも止まらないことを明示する |
...
| CURRENT | 下書き復元とフォロー中の記録はある。通知センターP-073は未実装 |
