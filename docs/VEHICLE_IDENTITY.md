...
P0 / Closed Alphaでは、登録済み車両から利用者がメーカー、車名、グレード、型式、根拠を提案できます。提案は提出者本人と運営だけが確認でき、運営確認前に共有カタログや既存の愛車表示へ自動反映しません。採用時は、既存項目の別名、新しいブランド、新しいモデル系統と市場別販売名等を分けて登録します。

カタログの照合では、運営が`exact`として公開した表記だけを安全な自動正規化に使います。歴史的会社名、関係が曖昧な表記、推測候補は`candidate_only`として提示に留めます。たとえば、`MAZDA`、`EUNOS`、`ɛ̃FINI`、`AUTOZAM`は同じ企業との関係を持てても別ブランドとして扱い、`富士重工業`は歴史的会社名として`SUBARU`への無確認変換を行いません。モデル名へ入力された`GTI 1.9`等の仕様語も、車名の別名ではなく世代、派生系統、具体仕様の候補として分離します。

P0の共有カタログはSupabase上のPostgreSQLテーブルを利用しますが、カタログ照合と入力検証は`packages/core`へ分離し、Provider固有APIを正本にしません。DBが未適用または取得不能な場合も、静的な小規模辞書と自由入力で愛車登録を継続できます。
