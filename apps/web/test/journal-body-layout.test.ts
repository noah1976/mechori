...
test("body media is full width on mobile and applies aspect bounds only on desktop", () => {
  assert.match(css, /\.journal-detail \{ --journal-body-media-bleed: 0px; padding: 0 0 34px; background: transparent; border: 0; \}/);
  ...
  assert.match(css, /\.journal-detail \{ --journal-body-media-bleed: 0px; padding: 0 0 28px; \}/);
