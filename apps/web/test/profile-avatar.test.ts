...
const garageIdentity = readFileSync(new URL("../components/garage-vehicle-identity.tsx", import.meta.url), "utf8");
...
test("Garage connects the current owner profile to the shared avatar", () => {
  assert.match(garage, /import \{ GarageVehicleIdentity \} from "@\/components\/garage-vehicle-identity"/);
  assert.match(garage, /ownerImagePath=\{owner\?\.profileImagePath\}/);
  assert.match(garageIdentity, /import \{ ProfileAvatar \} from "@\/components\/profile-avatar"/);
  assert.match(garageIdentity, /imagePath=\{ownerImagePath\}/);
});
