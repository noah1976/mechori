...
export function ProfessionalAdminPage() {
  ...
  const [ownerCandidates, setOwnerCandidates] = useState<ProfessionalMemberCandidate[]>([]);
  const [owner, setOwner] = useState<ProfessionalMemberCandidate | null>(null);
  ...
  async function createOrganization() {
    if (!name.trim() || !slug.trim() || !owner || busy) return;
    setBusy(true);
    setError(false);
    try {
      const id = await createProfessionalOrganization({ name: name.trim(), slug: slug.trim(), foundingGarage: founding, providerId: provider?.id, ownerUserId: owner.userId });
  ...
  async function searchInitialOwners() {
    if (!ownerQuery.trim()) return;
    setBusy(true);
    setError(false);
    try { setOwnerCandidates(await searchProfessionalMemberCandidates(undefined, ownerQuery)); }
    catch { setError(true); }
    finally { setBusy(false); }
  }
  ...
  return <div className="page-stack admin-page">
    ...
    <section className="admin-section professional-create-section"><header><div><span className="eyebrow">CREATE</span><h2>{ja ? "事業者を作成" : "Create organization"}</h2></div><Plus size={22} /></header><div className="form-grid two-columns"><label className="field"><span>{ja ? "事業者名" : "Name"}</span><input value={name} maxLength={120} onChange={(event) => setName(event.target.value)} /></label><label className="field"><span>Slug</span><input value={slug} maxLength={64} pattern="[a-z0-9-]+" onChange={(event) => setSlug(event.target.value.toLowerCase())} /></label></div><label className="checkbox-row"><input type="checkbox" checked={founding} onChange={(event) => setFounding(event.target.checked)} /><span>Founding Garage</span></label><div className="professional-provider-admin"><strong>{ja ? "初期OWNER" : "Initial OWNER"}</strong><p>{ja ? "事業者の作成には、少なくとも1人のOWNERが必要です。" : "Every organization must start with at least one OWNER."}</p>{owner && <div className="selected-provider"><ProfileAvatar displayName={owner.displayName} imagePath={owner.profileImagePath} /><span><strong>{owner.displayName}</strong><small>{owner.publicUsername ? `@${owner.publicUsername}` : ""}</small></span><button type="button" className="text-button" disabled={busy} onClick={() => setOwner(null)}>{ja ? "変更" : "Change"}</button></div>}<div className="inline-search"><input value={ownerQuery} placeholder={ja ? "表示名・@usernameで検索" : "Search name or @username"} onChange={(event) => setOwnerQuery(event.target.value)} /><button type="button" className="secondary-action" disabled={busy || !ownerQuery.trim()} onClick={() => void searchInitialOwners()}><Search size={17} />{ja ? "検索" : "Search"}</button></div>{ownerCandidates.length > 0 && <div className="professional-candidate-list">{ownerCandidates.map((candidate) => <button type="button" key={candidate.userId} onClick={() => { setOwner(candidate); setOwnerCandidates([]); }}><ProfileAvatar displayName={candidate.displayName} imagePath={candidate.profileImagePath} /><span><strong>{candidate.displayName}</strong><small>{candidate.publicUsername ? `@${candidate.publicUsername}` : ""}</small></span></button>)}</div>}</div><div className="professional-provider-admin"><strong>{ja ? "お店・工場を連携（任意）" : "Link provider (optional)"}</strong>{provider && <p>{provider.displayName}{provider.locality ? ` · ${provider.locality}` : ""}</p>}<div className="inline-search"><input value={providerQuery} placeholder={ja ? "店名・市区町村" : "Name or locality"} onChange={(event) => setProviderQuery(event.target.value)} /><button type="button" className="secondary-action" disabled={busy || !providerQuery.trim()} onClick={() => void searchProvidersForOrganization()}><Search size={17} />{ja ? "検索" : "Search"}</button></div>{providers.length > 0 && <div className="service-provider-results">{providers.map((item) => <button type="button" key={item.id} onClick={() => { setProvider(item); setProviders([]); }}><strong>{item.displayName}</strong><small>{item.locality}</small></button>)}</div>}</div><button type="button" className="primary-action" disabled={busy || !name.trim() || !slug.trim() || !owner} onClick={() => void createOrganization()}>{busy && <LoaderCircle className="spin" size={17} />}{ja ? "事業者を作成" : "Create organization"}</button></section>
