...
export default function VehicleSharePage() {
  ...
  const [shareError, setShareError] = useState<PublicVehicleShareErrorKind | null>(null);
  ...
  useEffect(() => {
    ...
    void loadOwnVehicleShare(vehicle.id)
      .then((result) => { if (active) { setShare(result); setShareError(null); setStatus("idle"); } })
      .catch((error: unknown) => {
        if (active) {
          setShareError(classifyPublicVehicleShareError(error));
          setStatus("error");
        }
      });
    return () => { active = false; };
    ...
    <div className="page-stack share-builder-page">
      ...
      <div className="share-builder-grid">
        ...
        <aside className="share-controls">
          ...
          {!remoteAlpha && <p className="form-error-summary">{ja ? "共有機能は現在の環境では利用できません。愛車の非公開データには影響ありません。" : "Sharing is unavailable in this environment. Your private vehicle data is unaffected."}</p>}
          {status === "error" && shareError && <p className="form-error-summary" role="alert">{publicVehicleShareErrorMessage(shareError, ja)}</p>}
